...
decade_loudness <- (ggplot(data_df,
                  ...
                  + stat_boxplot(geom = "errorbar", width=.25) 
                  + geom_boxplot() + xlab('Decade') 
                  + ylab('Loudness Scale') + ggtitle('Loudness Through the Decades')
)
...
grid.arrange(decade_danceabilaty, decade_energy, decade_speechiness,
             decade_duration, decade_valence, decade_liveness,
             ncol=3)
grid.arrange(decade_instrumentalness, decade_key, decade_acousticness,
             decade_tempo, decade_loudness, decade_time_signature)

#Bar graph of key with matching characters
...
data_features <- data_clean%>%
  select(-c(Song.Title, Artist, Year, key_character, n, Genre,
...
#Instrumnetalness and speechness are very simillar and it does
#not feel like it is necessary to have both
#and drop the n column that was created
data_clean2<-data_clean2%>%
  select(-c(instrumentalness, n))
